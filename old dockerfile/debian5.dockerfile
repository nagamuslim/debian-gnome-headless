...
RUN apt-get update && HOME=/home/debian apt-get install -y \
    ...
    $(apt show gnome-core | awk 'BEGIN{RS="";FS="\n"}{for(i=1;i<=NF;i++){if($i~/^Depends: /){line=$i;for(j=i+1;j<=NF&&$j~/^ /;j++){sub(/^ +/," ",$j);line=line $j};\sub(/^Depends: /,"",line);gsub(/\([^)]*\)/,"",line);gsub(/,/," ",line);gsub(/\|/," ",line);gsub(/ +/," ",line);sub(/^ /,"",line);sub(/ $/,"",line);n=split(line,arr," ");for(k=1;k<=n;k++){if(arr[k]!=""){print arr[k]}};exit}}}' | \grep -v '^pipewire-audio$' | tr '\n' ' ' | sed 's/ $//') \
    $(apt show gnome-core | awk 'BEGIN{RS="";FS="\n"}{for(i=1;i<=NF;i++){if($i~/^Recommends: /){line=$i;for(j=i+1;j<=NF&&$j~/^ /;j++){sub(/^ +/," ",$j);line=line $j};\sub(/^Recommends: /,"",line);gsub(/\([^)]*\)/,"",line);gsub(/,/," ",line);gsub(/\|/," ",line);gsub(/ +/," ",line);sub(/^ /,"",line);sub(/ $/,"",line);sub(/>$/,"",line);n=split(line,arr," ");for(k=1;k<=n;k++){if(arr[k]!=""){print arr[k]}};exit}}}' | \grep -v 'chromium' | grep -v '^firefox-esr$' | tr '\n' ' ' | sed 's/ $//') \
    $(apt show gnome-core | awk 'BEGIN{RS="";FS="\n"}{for(i=1;i<=NF;i++){if($i~/^Suggests: /){line=$i;for(j=i+1;j<=NF&&$j~/^ /;j++){sub(/^ +/," ",$j);line=line $j};\sub(/^Suggests: /,"",line);gsub(/\([^)]*\)/,"",line);gsub(/,/," ",line);gsub(/\|/," ",line);gsub(/ +/," ",line);sub(/^ /,"",line);sub(/ $/,"",line);sub(/>$/,"",line);n=split(line,arr," ");for(k=1;k<=n;k++){if(arr[k]!=""){print arr[k]}};exit}}}' | \grep -v '^gnome$' | tr '\n' ' ' | sed 's/ $//' \
    $(apt show gnome | awk 'BEGIN{RS="";FS="\n"}{for(i=1;i<=NF;i++){if($i~/^Depends: /){line=$i;for(j=i+1;j<=NF&&$j~/^ /;j++){sub(/^ +/," ",$j);line=line $j};\sub(/^Depends: /,"",line);gsub(/\([^)]*\)/,"",line);gsub(/,/," ",line);gsub(/\|/," ",line);gsub(/ +/," ",line);sub(/^ /,"",line);sub(/ $/,"",line);sub(/>$/,"",line);n=split(line,arr," ");for(k=1;k<=n;k++){if(arr[k]!=""){print arr[k]}};exit}}}' | \grep -v '^gnome-core$' | tr '\n' ' ' | sed 's/ $//') \
    $(apt show gnome | awk 'BEGIN{RS="";FS="\n"}{for(i=1;i<=NF;i++){if($i~/^Recommends: /){line=$i;for(j=i+1;j<=NF&&$j~/^ /;j++){sub(/^ +/," ",$j);line=line $j};\sub(/^Recommends: /,"",line);gsub(/\([^)]*\)/,"",line);gsub(/,/," ",line);gsub(/\|/," ",line);gsub(/ +/," ",line);sub(/^ /,"",line);sub(/ $/,"",line);sub(/>$/,"",line);n=split(line,arr," ");for(k=1;k<=n;k++){if(arr[k]!=""){print arr[k]}};exit}}}' | \tr '\n' ' ' | sed 's/ $//') \
    $(apt show gnome | awk 'BEGIN{RS="";FS="\n"}{for(i=1;i<=NF;i++){if($i~/^Suggests: /){line=$i;for(j=i+1;j<=NF&&$j~/^ /;j++){sub(/^ +/," ",$j);line=line $j};\sub(/^Suggests: /,"",line);gsub(/\([^)]*\)/,"",line);gsub(/,/," ",line);gsub(/\|/," ",line);gsub(/ +/," ",line);sub(/^ /,"",line);sub(/ $/,"",line);sub(/>$/,"",line);n=split(line,arr," ");for(k=1;k<=n;k++){if(arr[k]!=""){print arr[k]}};exit}}}' | \grep -v '^firefox-.*-all$' | grep -v '^webext-ublock-origin-chromium$' | tr '\n' ' ' | sed 's/ $//') \
    pulseaudio \
    gnome-settings-* \
    gnome-terminal-* \
