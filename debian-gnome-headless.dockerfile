...
RUN echo -e "\nload-module module-simple-protocol-tcp listen=0.0.0.0 source=auto_null.monitor record=true port=4713 rate=48000\nload-module module-x11-publish display=:1\nload-module module-x11-cork-request display=:1" >> /etc/pulse/system.pa && sed -i '/-e/d' /etc/pulse/system.pa && sed -i '/-e/d' /home/debian/butt.txt || true && \
