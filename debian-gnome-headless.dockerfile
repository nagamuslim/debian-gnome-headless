...
#COPY *.desktop /home/debian/Desktop/
    ...
    mkdir -p ~/.config/tigervnc && cp ~/.vnc/* ~/.config/tigervnc/ && sudo chmod 755 /home/debian && sudo cp /home/debian/mediamtx.yml /mediamtx.yml  && grep -q 'gnome-extensions enable ding@rastersoft.com' /home/debian/.vnc/xstartup && exit 0 || ( sed -i '/gsettings set org.gnome.desktop.background show-desktop-icons true/a\gsettings set org.gnome.desktop.screensaver lock-enabled false' /home/debian/.vnc/xstartup && sed -i '/gsettings set org.gnome.desktop.background show-desktop-icons true/a\gnome-extensions enable ding@rastersoft.com' /home/debian/.vnc/xstartup && sed -i '/gsettings set org.gnome.desktop.background show-desktop-icons true/a\( current_lang=$(localectl status | grep "System Locale" | cut -d'=' -f2); while sleep 10; do new_lang=$(localectl status | grep "System Locale" | cut -d'=' -f2); sys_lang=$(cat /etc/default/locale 2>/dev/null | grep LANG= | cut -d'=' -f2 | tr -d '\"') ; [ "$current_lang" != "$new_lang" ] || [ "$current_lang" != "$sys_lang" ] && echo "export DEFAULT_LANG='$new_lang'" | sudo tee /etc/profile.d/00docker-env.sh > /dev/null && kill $(cat /home/debian/.vnc/$(hostname):1.pid) && exit; current_lang="$new_lang"; done ) &' /home/debian/.vnc/xstartup )
