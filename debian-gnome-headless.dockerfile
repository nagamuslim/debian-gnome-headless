...
RUN echo "load-module module-simple-protocol-tcp listen=0.0.0.0 source=auto_null.monitor record=true port=4713 rate=48000" >> /etc/pulse/system.pa && echo "load-module module-x11-publish display=:1" >> /etc/pulse/system.pa && echo "load-module module-x11-cork-request display=:1" >> /etc/pulse/system.pa && sed -i '/-e/d' /home/debian/butt.txt || true && \
